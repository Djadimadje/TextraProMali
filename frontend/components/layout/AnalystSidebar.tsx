...
const AnalystSidebar: React.FC<AnalystSidebarProps> = ({ className = '' }) => {
  const pathname = usePathname();
  const { user, logout } = useAuth();
  
  const menuItems = [
    { key: 'dashboard', label: 'Dashboard', href: '/analyst', icon: 'LayoutDashboard', description: 'Analyst overview' },
    { key: 'analytics', label: 'Analytics', href: '/analyst/analytics', icon: 'BarChart', description: 'Performance analysis' },
    { key: 'reports', label: 'Reports', href: '/analyst/reports', icon: 'FileText', description: 'Generate & view reports' },
    { key: 'workflow', label: 'Workflow', href: '/analyst/workflow', icon: 'Workflow', description: 'Analyze workflow data' },
    { key: 'machines', label: 'Machines', href: '/analyst/machines', icon: 'Cog', description: 'Analyze machine data' },
    { key: 'notifications', label: 'Notifications', href: '/analyst/notifications', icon: 'Bell', description: 'System alerts' }
  ];
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">
                {user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.username : 'Analyst'}
              </span>
              <span className="text-xs text-indigo-600 font-medium">
                {user?.role === 'analyst' ? 'Data Analyst' : user?.role || 'Analyst'}
              </span>
            </div>
          </div>
        </div>
