...
const TechnicianSidebar: React.FC<TechnicianSidebarProps> = ({ className = '' }) => {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">
                {user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.username : 'Technician'}
              </span>
              <span className="text-xs text-indigo-600 font-medium">
                {user?.role === 'technician' ? 'Maintenance Technician' : user?.role || 'Technician'}
              </span>
            </div>
          </div>
        </div>
