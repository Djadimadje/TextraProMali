...
const InspectorSidebar: React.FC<InspectorSidebarProps> = ({ className = '' }) => {
  const pathname = usePathname();
  const { user, logout } = useAuth();
  
  const menuItems = [
    { key: 'dashboard', label: 'Dashboard', href: '/inspector', icon: 'LayoutDashboard', description: 'Inspector overview' },
    { key: 'quality', label: 'Quality Control', href: '/inspector/quality', icon: 'CheckCircle', description: 'Inspect textile quality' },
    { key: 'machines', label: 'Machines', href: '/inspector/machines', icon: 'Cog', description: 'Inspect machine performance' },
    { key: 'notifications', label: 'Notifications', href: '/inspector/notifications', icon: 'Bell', description: 'Inspection alerts' }
  ];
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">
                {user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.username : 'Inspector'}
              </span>
              <span className="text-xs text-indigo-600 font-medium">
                {user?.role === 'inspector' ? 'Quality Inspector' : user?.role || 'Inspector'}
              </span>
            </div>
          </div>
        </div>
