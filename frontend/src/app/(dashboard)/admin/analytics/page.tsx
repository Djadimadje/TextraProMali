...
export default function AnalyticsPage() {
  ...
  const loadAnalyticsData = async () => {
    try {
      console.log('loadAnalyticsData started');
      setLoading(true);
      setError(null);
      ...
      if (productionResult.success) {
        ...
        console.log('Production data loaded:', productionResult.data);
      } else {
        console.error('Production analytics failed:', productionResult.error);
      }
    ...
    try {
      ...
      switch (type) {
        case 'production':
          if (productionData) {
            data = [
              { metric: 'Total Batches', value: productionData.total_batches },
              { metric: 'Efficiency', value: `${productionData.efficiency}%` },
              { metric: 'Completed', value: `${productionData.percentages?.completed || 0}%` },
              { metric: 'In Progress', value: `${productionData.percentages?.in_progress || 0}%` },
              { metric: 'Pending', value: `${productionData.percentages?.pending || 0}%` },
            ];
          }
          filename = 'production_analytics';
        ...
        case 'machine':
          if (machineData) {
            data = [
              { metric: 'Total Machines', value: machineData.total_machines },
              { metric: 'Utilization Rate', value: `${machineData.utilization_rate}%` },
              { metric: 'Operational', value: `${machineData.percentages?.operational || 0}%` },
              { metric: 'Maintenance', value: `${machineData.percentages?.maintenance || 0}%` },
              { metric: 'Idle', value: `${machineData.percentages?.idle || 0}%` },
            ];
          }
          filename = 'machine_analytics';
        ...
        case 'quality':
          if (qualityData) {
            data = [
              { metric: 'Total Checks', value: qualityData.total_checks },
              { metric: 'Approval Rate', value: `${qualityData.percentages?.approved || 0}%` },
              { metric: 'AI Accuracy', value: `${qualityData.ai_accuracy}%` },
              { metric: 'Approved', value: `${qualityData.percentages?.approved || 0}%` },
              { metric: 'Rejected', value: `${qualityData.percentages?.rejected || 0}%` },
              { metric: 'Pending', value: `${qualityData.percentages?.pending || 0}%` },
            ];
          }
          filename = 'quality_analytics';
          title = 'Quality Analytics Report';
          break;
